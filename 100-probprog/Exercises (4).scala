...
object Exercises {
  ...
  def pick (n: Int): Element[Boolean] = {
    // you have 8 balls. you pick one. what can be the outcome?
    // the ball is red
    // the ball is black
    // The sample space is S = {red, black}
    // P(R) = 1/8
    // P(B) = 7/8
    val red = Flip(1.0 / (n + 1.0))
    red
  }
  ...
  def move (player: Player, n: Int): Element[Player] = for {
    // player 1 picks a ball
    red <- pick(n)
    winner <- if (red) Constant(player)
    else move(next(player), n - 1)
  } yield(winner)
  ...
  def probPaula: Double = {
    val winner = move(Paula, BallsNo - 1)
    Importance.probability(winner, Paula)
  }

  // Probability that Paula wins given Peter starts (the total no of balls: BallsNo)
  def probPeter: Double = {
    val winner = move(Peter, BallsNo - 1)
    1 - Importance.probability(winner, Peter)
  }
  ...
  //
  // The probability that Paula wins is higher if she starts.
  // If BallsNo == 9, it is not important who starts.
  ...
  def gameResult: Element[Player] = {
    val isPaula = firstMover map (p => p == Paula)
    val isPaulaProb = Importance.probability(isPaula, true)
    if (isPaulaProb == 1) move(Paula, BallsNo - 1)
    else move(Peter, BallsNo - 1)
  }
  ...
  def probPaulaStarted: Double = {
    (probPaula * 0.5) / Importance.probability(gameResult, Paula)
  }
  ...
  //
  // Yes, it depends. If it's even, she has slightly less chances to win rather than if it was odd.
  // BallsNo = 8 => probPaulaStarted = 0.49522388059701206
  // BallsNo = 9 => probPaulaStarted = 0.6290069491145461
  ...
  // For us it was 0.5017000000000083

  // Reflect whether the above estimation would take you more time analytically
  // or with a probabilistic programming library?
  // We believe that it takes time to learn how to work with such a library,
  // but once you get a hold of it, it will definitely be quicker
