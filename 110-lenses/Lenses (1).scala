...
// // Group number: Work-Group 23
// //
// // AUTHOR1: Dana-Maria Iliescu (dail@itu.dk)
// // TIME1: 5h <- how much time have you used on solving this exercise set
// // (excluding reading the book, fetching pizza, and going out for a smoke)
// //
// // AUTHOR2: Sara Qirko (saqi@itu.dk)
// // TIME2: 5h <- how much time have you used on solving this exercise set
// // (excluding reading the book, fetching pizza, and going out for a smoke)
// //
// // AUTHOR2: Petr Sindelar (pesi@itu.dk)
// // TIME2: 5h <- how much time have you used on solving this exercise set
// // (excluding reading the book, fetching pizza, and going out for a smoke)
...
//     Lens[(String,Int), String](_._1) (s => (t:(String,Int)) => if (s == t._1) (s, t._2) else (s, t._2 + 1))
...
//   def index(name: String): Optional[Map[String,Address],Address] =
//     Optional[Map[String,Address],Address](_.get(name))(a => s => s + (name -> a))
...
//   val composedOptic = _students ^|-? index("Alex") ^|-> _zipcode
//   val itu2 :University = composedOptic.set("9100")(itu)
...
//   //
//   val composedOpticTraversal = _students ^|->> each ^|-> _country
//   val itu3 :University = composedOpticTraversal.modify(_.toUpperCase)(itu)
...
//   // the answer below:
//   //
//   // The test uses "forAll" instead of "each". In the test, we already have the itu3
//   // value and we can check each country with "forAll", but here we need to focus on the
//   // country property and return a new value with the modified value.
...
//   val composedOpticTraversal2 = _students ^|->> filterIndex { name: String => name(0).equals('A') } ^|-> _country
//   val itu4 :University = composedOpticTraversal2.modify(_.toUpperCase)(itu)
...
//     Optional[List[A],A](_.lift(n))(a => as => if (n >= as.length || n < 0) as else as.updated(n, a))
...
//   def setIth1[A] (n: Integer, default: A) :Lens[List[A],A] = Lens[List[A],A] ((as: List[A]) => if (n >= as.length) default else as(n)) ((a: A) => (as: List[A]) => if (n >= as.length) {
//     val diff: List[A] = List.fill(n - as.length)(default)
//     as ++ diff ++ List(a)
//   } else {
//     as.updated(n, a)
//   })
...
//   val list1 = setIth[Int](2).set(list0(2) + 1)(list0)
//   // println (list0)
//   // println (list1)
