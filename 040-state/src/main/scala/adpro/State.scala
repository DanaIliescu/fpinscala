...
object RNG {
  ...
  // Exercise 3 (CB 6.3)
  // Write functions to generate an (Int, Double) pair and a (Double, Int) pair, where integers are non-negative.
...
object State {
  ...
  // Exercise 10
  // Implement a function state2stream that given a state object and an initial state,
  // produces a stream of values generated by this State object (this automaton).
  def state2stream[S,A] (s :State[S,A]) (seed: S) :Stream[A] = {
    def loop(s: State[S, A], seed: S): Stream[A] = {
      val (a, s2) = s.run(seed)
      a #:: loop(s, s2)
    }
    loop(s, seed)
  ...
  // Use state2stream to generate a lazy stream of integer numbers. Finally, obtain a
  // finite list of 10 random values from this stream.
  lazy val random_integers: Stream[Int] = state2stream(State.random_int)(RNG.SimpleRNG(42))
